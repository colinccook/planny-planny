...
Access levels and role capabilities.
...
Capture these states at the mobile viewport (390×844) and add the resulting images under `docs/screenshots/permissions/`:
...
The Playwright BDD scenarios are the source of truth for the journey. A scenario can save a snapshot with `await page.screenshot({ path: 'docs/screenshots/permissions/04-edge-case.png', fullPage: true })`; keep screenshots deterministic and refresh them when the behaviour changes.

## Tests and implementation

- [Search all `@Permissions` references](https://github.com/colinccook/planny-planny/search?q=@Permissions&type=code)
