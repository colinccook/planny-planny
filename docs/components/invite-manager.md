...
Reusable UI documentation for **!InviteManager**.
...
- [Authoritative source: `InviteManager.tsx`](../src/components/settings/InviteManager.tsx)
- [Search all `!InviteManager` references](https://github.com/colinccook/planny-planny/search?q=!InviteManager&type=code)
- [Find tests that render or drive it](https://github.com/colinccook/planny-planny/search?q=!InviteManager+repo%3Acolinccook%2Fplanny-planny&type=code)
...
Document screenshots for each state that users can encounter. Capture at 390×844 and store them under `docs/screenshots/components/invite-manager/`.
...
Playwright tests can create these documentation snapshots with `await page.screenshot({ path: 'docs/screenshots/components/invite-manager/03-active.png', fullPage: true })`. Keep the test setup deterministic, avoid credentials in screenshots, and update images when the observable states change.
