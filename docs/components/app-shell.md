...
Reusable UI documentation for **!AppShell**.
...
- [Authoritative source: `AppShell.tsx`](../src/components/layout/AppShell.tsx)
- [Search all `!AppShell` references](https://github.com/colinccook/planny-planny/search?q=!AppShell&type=code)
- [Find tests that render or drive it](https://github.com/colinccook/planny-planny/search?q=!AppShell+repo%3Acolinccook%2Fplanny-planny&type=code)
...
Document screenshots for each state that users can encounter. Capture at 390×844 and store them under `docs/screenshots/components/app-shell/`.
...
Playwright tests can create these documentation snapshots with `await page.screenshot({ path: 'docs/screenshots/components/app-shell/03-active.png', fullPage: true })`. Keep the test setup deterministic, avoid credentials in screenshots, and update images when the observable states change.
