...
Reusable UI documentation for **!DayContextBadge**.
...
- [Authoritative source: `DayContextBadge.tsx`](../src/components/calendar/DayContextBadge.tsx)
- [Search all `!DayContextBadge` references](https://github.com/colinccook/planny-planny/search?q=!DayContextBadge&type=code)
- [Find tests that render or drive it](https://github.com/colinccook/planny-planny/search?q=!DayContextBadge+repo%3Acolinccook%2Fplanny-planny&type=code)
...
Document screenshots for each state that users can encounter. Capture at 390×844 and store them under `docs/screenshots/components/day-context-badge/`.
...
Playwright tests can create these documentation snapshots with `await page.screenshot({ path: 'docs/screenshots/components/day-context-badge/03-active.png', fullPage: true })`. Keep the test setup deterministic, avoid credentials in screenshots, and update images when the observable states change.
