...
Reusable UI documentation for **!SuccessfulMealsHeadline**.
...
- [Authoritative source: `SuccessfulMealsHeadline.tsx`](../src/components/auth/SuccessfulMealsHeadline.tsx)
- [Search all `!SuccessfulMealsHeadline` references](https://github.com/colinccook/planny-planny/search?q=!SuccessfulMealsHeadline&type=code)
- [Find tests that render or drive it](https://github.com/colinccook/planny-planny/search?q=!SuccessfulMealsHeadline+repo%3Acolinccook%2Fplanny-planny&type=code)
...
Document screenshots for each state that users can encounter. Capture at 390×844 and store them under `docs/screenshots/components/successful-meals-headline/`.
...
Playwright tests can create these documentation snapshots with `await page.screenshot({ path: 'docs/screenshots/components/successful-meals-headline/03-active.png', fullPage: true })`. Keep the test setup deterministic, avoid credentials in screenshots, and update images when the observable states change.
