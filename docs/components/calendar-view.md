...
Reusable UI documentation for **!CalendarView**.
...
- [Authoritative source: `CalendarView.tsx`](../src/components/calendar/CalendarView.tsx)
- [Search all `!CalendarView` references](https://github.com/colinccook/planny-planny/search?q=!CalendarView&type=code)
- [Find tests that render or drive it](https://github.com/colinccook/planny-planny/search?q=!CalendarView+repo%3Acolinccook%2Fplanny-planny&type=code)
...
Document screenshots for each state that users can encounter. Capture at 390×844 and store them under `docs/screenshots/components/calendar-view/`.
...
Playwright tests can create these documentation snapshots with `await page.screenshot({ path: 'docs/screenshots/components/calendar-view/03-active.png', fullPage: true })`. Keep the test setup deterministic, avoid credentials in screenshots, and update images when the observable states change.
