...
Reusable UI documentation for **!TodoList**.
...
- [Authoritative source: `TodoList.tsx`](../src/components/calendar/TodoList.tsx)
- [Search all `!TodoList` references](https://github.com/colinccook/planny-planny/search?q=!TodoList&type=code)
- [Find tests that render or drive it](https://github.com/colinccook/planny-planny/search?q=!TodoList+repo%3Acolinccook%2Fplanny-planny&type=code)
...
Document screenshots for each state that users can encounter. Capture at 390×844 and store them under `docs/screenshots/components/todo-list/`.
...
Playwright tests can create these documentation snapshots with `await page.screenshot({ path: 'docs/screenshots/components/todo-list/03-active.png', fullPage: true })`. Keep the test setup deterministic, avoid credentials in screenshots, and update images when the observable states change.
