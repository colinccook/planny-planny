...
Reusable UI documentation for **!AccessLevelsLink**.
...
- [Authoritative source: `AccessLevelsLink.tsx`](../src/components/settings/AccessLevelsLink.tsx)
- [Search all `!AccessLevelsLink` references](https://github.com/colinccook/planny-planny/search?q=!AccessLevelsLink&type=code)
- [Find tests that render or drive it](https://github.com/colinccook/planny-planny/search?q=!AccessLevelsLink+repo%3Acolinccook%2Fplanny-planny&type=code)
...
Document screenshots for each state that users can encounter. Capture at 390×844 and store them under `docs/screenshots/components/access-levels-link/`.
...
Playwright tests can create these documentation snapshots with `await page.screenshot({ path: 'docs/screenshots/components/access-levels-link/03-active.png', fullPage: true })`. Keep the test setup deterministic, avoid credentials in screenshots, and update images when the observable states change.
