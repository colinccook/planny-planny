...
Reusable UI documentation for **!MemberList**.
...
- [Authoritative source: `MemberList.tsx`](../src/components/settings/MemberList.tsx)
- [Search all `!MemberList` references](https://github.com/colinccook/planny-planny/search?q=!MemberList&type=code)
- [Find tests that render or drive it](https://github.com/colinccook/planny-planny/search?q=!MemberList+repo%3Acolinccook%2Fplanny-planny&type=code)
...
Document screenshots for each state that users can encounter. Capture at 390×844 and store them under `docs/screenshots/components/member-list/`.
...
Playwright tests can create these documentation snapshots with `await page.screenshot({ path: 'docs/screenshots/components/member-list/03-active.png', fullPage: true })`. Keep the test setup deterministic, avoid credentials in screenshots, and update images when the observable states change.
