...
Reusable UI documentation for **!Toast**.
...
- [Authoritative source: `Toast.tsx`](../src/components/ui/Toast.tsx)
- [Search all `!Toast` references](https://github.com/colinccook/planny-planny/search?q=!Toast&type=code)
- [Find tests that render or drive it](https://github.com/colinccook/planny-planny/search?q=!Toast+repo%3Acolinccook%2Fplanny-planny&type=code)
...
Document screenshots for each state that users can encounter. Capture at 390×844 and store them under `docs/screenshots/components/toast/`.
...
Playwright tests can create these documentation snapshots with `await page.screenshot({ path: 'docs/screenshots/components/toast/03-active.png', fullPage: true })`. Keep the test setup deterministic, avoid credentials in screenshots, and update images when the observable states change.
