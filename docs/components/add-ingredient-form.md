...
Reusable UI documentation for **!AddIngredientForm**.
...
- [Authoritative source: `AddIngredientForm.tsx`](../src/components/ingredients/AddIngredientForm.tsx)
- [Search all `!AddIngredientForm` references](https://github.com/colinccook/planny-planny/search?q=!AddIngredientForm&type=code)
- [Find tests that render or drive it](https://github.com/colinccook/planny-planny/search?q=!AddIngredientForm+repo%3Acolinccook%2Fplanny-planny&type=code)
...
Document screenshots for each state that users can encounter. Capture at 390×844 and store them under `docs/screenshots/components/add-ingredient-form/`.
...
Playwright tests can create these documentation snapshots with `await page.screenshot({ path: 'docs/screenshots/components/add-ingredient-form/03-active.png', fullPage: true })`. Keep the test setup deterministic, avoid credentials in screenshots, and update images when the observable states change.
