...
Reusable UI documentation for **!OverlayProvider**.
...
- [Authoritative source: `OverlayProvider.tsx`](../src/components/ui/OverlayProvider.tsx)
- [Search all `!OverlayProvider` references](https://github.com/colinccook/planny-planny/search?q=!OverlayProvider&type=code)
- [Find tests that render or drive it](https://github.com/colinccook/planny-planny/search?q=!OverlayProvider+repo%3Acolinccook%2Fplanny-planny&type=code)
...
Document screenshots for each state that users can encounter. Capture at 390×844 and store them under `docs/screenshots/components/overlay-provider/`.
...
Playwright tests can create these documentation snapshots with `await page.screenshot({ path: 'docs/screenshots/components/overlay-provider/03-active.png', fullPage: true })`. Keep the test setup deterministic, avoid credentials in screenshots, and update images when the observable states change.
