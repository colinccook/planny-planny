...
Reusable UI documentation for **!OutcomeTray**.
...
- [Authoritative source: `OutcomeTray.tsx`](../src/components/calendar/OutcomeTray.tsx)
- [Search all `!OutcomeTray` references](https://github.com/colinccook/planny-planny/search?q=!OutcomeTray&type=code)
- [Find tests that render or drive it](https://github.com/colinccook/planny-planny/search?q=!OutcomeTray+repo%3Acolinccook%2Fplanny-planny&type=code)
...
Document screenshots for each state that users can encounter. Capture at 390×844 and store them under `docs/screenshots/components/outcome-tray/`.
...
Playwright tests can create these documentation snapshots with `await page.screenshot({ path: 'docs/screenshots/components/outcome-tray/03-active.png', fullPage: true })`. Keep the test setup deterministic, avoid credentials in screenshots, and update images when the observable states change.
