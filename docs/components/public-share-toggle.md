...
Reusable UI documentation for **!PublicShareToggle**.
...
- [Authoritative source: `PublicShareToggle.tsx`](../src/components/settings/PublicShareToggle.tsx)
- [Search all `!PublicShareToggle` references](https://github.com/colinccook/planny-planny/search?q=!PublicShareToggle&type=code)
- [Find tests that render or drive it](https://github.com/colinccook/planny-planny/search?q=!PublicShareToggle+repo%3Acolinccook%2Fplanny-planny&type=code)
...
Document screenshots for each state that users can encounter. Capture at 390×844 and store them under `docs/screenshots/components/public-share-toggle/`.
...
Playwright tests can create these documentation snapshots with `await page.screenshot({ path: 'docs/screenshots/components/public-share-toggle/03-active.png', fullPage: true })`. Keep the test setup deterministic, avoid credentials in screenshots, and update images when the observable states change.
