...
Reusable UI documentation for **!SwipeableDay**.
...
- [Authoritative source: `SwipeableDay.tsx`](../src/components/calendar/SwipeableDay.tsx)
- [Search all `!SwipeableDay` references](https://github.com/colinccook/planny-planny/search?q=!SwipeableDay&type=code)
- [Find tests that render or drive it](https://github.com/colinccook/planny-planny/search?q=!SwipeableDay+repo%3Acolinccook%2Fplanny-planny&type=code)
...
Document screenshots for each state that users can encounter. Capture at 390×844 and store them under `docs/screenshots/components/swipeable-day/`.
...
Playwright tests can create these documentation snapshots with `await page.screenshot({ path: 'docs/screenshots/components/swipeable-day/03-active.png', fullPage: true })`. Keep the test setup deterministic, avoid credentials in screenshots, and update images when the observable states change.
