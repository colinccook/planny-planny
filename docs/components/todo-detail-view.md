...
Reusable UI documentation for **!TodoDetailView**.
...
- [Authoritative source: `TodoDetailView.tsx`](../src/components/calendar/TodoDetailView.tsx)
- [Search all `!TodoDetailView` references](https://github.com/colinccook/planny-planny/search?q=!TodoDetailView&type=code)
- [Find tests that render or drive it](https://github.com/colinccook/planny-planny/search?q=!TodoDetailView+repo%3Acolinccook%2Fplanny-planny&type=code)
...
Document screenshots for each state that users can encounter. Capture at 390×844 and store them under `docs/screenshots/components/todo-detail-view/`.
...
Playwright tests can create these documentation snapshots with `await page.screenshot({ path: 'docs/screenshots/components/todo-detail-view/03-active.png', fullPage: true })`. Keep the test setup deterministic, avoid credentials in screenshots, and update images when the observable states change.
