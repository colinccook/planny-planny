...
Reusable UI documentation for **!HouseholdSwitcher**.
...
- [Authoritative source: `HouseholdSwitcher.tsx`](../src/components/settings/HouseholdSwitcher.tsx)
- [Search all `!HouseholdSwitcher` references](https://github.com/colinccook/planny-planny/search?q=!HouseholdSwitcher&type=code)
- [Find tests that render or drive it](https://github.com/colinccook/planny-planny/search?q=!HouseholdSwitcher+repo%3Acolinccook%2Fplanny-planny&type=code)
...
Document screenshots for each state that users can encounter. Capture at 390×844 and store them under `docs/screenshots/components/household-switcher/`.
...
Playwright tests can create these documentation snapshots with `await page.screenshot({ path: 'docs/screenshots/components/household-switcher/03-active.png', fullPage: true })`. Keep the test setup deterministic, avoid credentials in screenshots, and update images when the observable states change.
