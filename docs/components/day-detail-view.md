...
Reusable UI documentation for **!DayDetailView**.
...
- [Authoritative source: `DayDetailView.tsx`](../src/components/calendar/DayDetailView.tsx)
- [Search all `!DayDetailView` references](https://github.com/colinccook/planny-planny/search?q=!DayDetailView&type=code)
- [Find tests that render or drive it](https://github.com/colinccook/planny-planny/search?q=!DayDetailView+repo%3Acolinccook%2Fplanny-planny&type=code)
...
Document screenshots for each state that users can encounter. Capture at 390×844 and store them under `docs/screenshots/components/day-detail-view/`.
...
Playwright tests can create these documentation snapshots with `await page.screenshot({ path: 'docs/screenshots/components/day-detail-view/03-active.png', fullPage: true })`. Keep the test setup deterministic, avoid credentials in screenshots, and update images when the observable states change.
