...
Reusable UI documentation for **!MyMemberships**.
...
- [Authoritative source: `MyMemberships.tsx`](../src/components/settings/MyMemberships.tsx)
- [Search all `!MyMemberships` references](https://github.com/colinccook/planny-planny/search?q=!MyMemberships&type=code)
- [Find tests that render or drive it](https://github.com/colinccook/planny-planny/search?q=!MyMemberships+repo%3Acolinccook%2Fplanny-planny&type=code)
...
Document screenshots for each state that users can encounter. Capture at 390×844 and store them under `docs/screenshots/components/my-memberships/`.
...
Playwright tests can create these documentation snapshots with `await page.screenshot({ path: 'docs/screenshots/components/my-memberships/03-active.png', fullPage: true })`. Keep the test setup deterministic, avoid credentials in screenshots, and update images when the observable states change.
