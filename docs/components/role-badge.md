...
Reusable UI documentation for **!RoleBadge**.
...
- [Authoritative source: `RoleBadge.tsx`](../src/components/settings/RoleBadge.tsx)
- [Search all `!RoleBadge` references](https://github.com/colinccook/planny-planny/search?q=!RoleBadge&type=code)
- [Find tests that render or drive it](https://github.com/colinccook/planny-planny/search?q=!RoleBadge+repo%3Acolinccook%2Fplanny-planny&type=code)
...
Document screenshots for each state that users can encounter. Capture at 390×844 and store them under `docs/screenshots/components/role-badge/`.
...
Playwright tests can create these documentation snapshots with `await page.screenshot({ path: 'docs/screenshots/components/role-badge/03-active.png', fullPage: true })`. Keep the test setup deterministic, avoid credentials in screenshots, and update images when the observable states change.
