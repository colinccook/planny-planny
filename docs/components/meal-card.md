...
Reusable UI documentation for **!MealCard**.
...
- [Authoritative source: `MealCard.tsx`](../src/components/calendar/MealCard.tsx)
- [Search all `!MealCard` references](https://github.com/colinccook/planny-planny/search?q=!MealCard&type=code)
- [Find tests that render or drive it](https://github.com/colinccook/planny-planny/search?q=!MealCard+repo%3Acolinccook%2Fplanny-planny&type=code)
...
Document screenshots for each state that users can encounter. Capture at 390×844 and store them under `docs/screenshots/components/meal-card/`.
...
Playwright tests can create these documentation snapshots with `await page.screenshot({ path: 'docs/screenshots/components/meal-card/03-active.png', fullPage: true })`. Keep the test setup deterministic, avoid credentials in screenshots, and update images when the observable states change.
