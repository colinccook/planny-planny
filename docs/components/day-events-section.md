...
Reusable UI documentation for **!DayEventsSection**.
...
- [Authoritative source: `DayEventsSection.tsx`](../src/components/calendar/DayEventsSection.tsx)
- [Search all `!DayEventsSection` references](https://github.com/colinccook/planny-planny/search?q=!DayEventsSection&type=code)
- [Find tests that render or drive it](https://github.com/colinccook/planny-planny/search?q=!DayEventsSection+repo%3Acolinccook%2Fplanny-planny&type=code)
...
Document screenshots for each state that users can encounter. Capture at 390×844 and store them under `docs/screenshots/components/day-events-section/`.
...
Playwright tests can create these documentation snapshots with `await page.screenshot({ path: 'docs/screenshots/components/day-events-section/03-active.png', fullPage: true })`. Keep the test setup deterministic, avoid credentials in screenshots, and update images when the observable states change.
