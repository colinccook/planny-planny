...
Reusable UI documentation for **!IngredientTag**.
...
- [Authoritative source: `IngredientTag.tsx`](../src/components/ingredients/IngredientTag.tsx)
- [Search all `!IngredientTag` references](https://github.com/colinccook/planny-planny/search?q=!IngredientTag&type=code)
- [Find tests that render or drive it](https://github.com/colinccook/planny-planny/search?q=!IngredientTag+repo%3Acolinccook%2Fplanny-planny&type=code)
...
Document screenshots for each state that users can encounter. Capture at 390×844 and store them under `docs/screenshots/components/ingredient-tag/`.
...
Playwright tests can create these documentation snapshots with `await page.screenshot({ path: 'docs/screenshots/components/ingredient-tag/03-active.png', fullPage: true })`. Keep the test setup deterministic, avoid credentials in screenshots, and update images when the observable states change.
