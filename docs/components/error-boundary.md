...
Reusable UI documentation for **!ErrorBoundary**.
...
- [Authoritative source: `ErrorBoundary.tsx`](../src/components/ui/ErrorBoundary.tsx)
- [Search all `!ErrorBoundary` references](https://github.com/colinccook/planny-planny/search?q=!ErrorBoundary&type=code)
- [Find tests that render or drive it](https://github.com/colinccook/planny-planny/search?q=!ErrorBoundary+repo%3Acolinccook%2Fplanny-planny&type=code)
...
Document screenshots for each state that users can encounter. Capture at 390×844 and store them under `docs/screenshots/components/error-boundary/`.
...
Playwright tests can create these documentation snapshots with `await page.screenshot({ path: 'docs/screenshots/components/error-boundary/03-active.png', fullPage: true })`. Keep the test setup deterministic, avoid credentials in screenshots, and update images when the observable states change.
