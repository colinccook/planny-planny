...
Reusable UI documentation for **!ProtectedRoute**.
...
- [Authoritative source: `ProtectedRoute.tsx`](../src/components/layout/ProtectedRoute.tsx)
- [Search all `!ProtectedRoute` references](https://github.com/colinccook/planny-planny/search?q=!ProtectedRoute&type=code)
- [Find tests that render or drive it](https://github.com/colinccook/planny-planny/search?q=!ProtectedRoute+repo%3Acolinccook%2Fplanny-planny&type=code)
...
Document screenshots for each state that users can encounter. Capture at 390×844 and store them under `docs/screenshots/components/protected-route/`.
...
Playwright tests can create these documentation snapshots with `await page.screenshot({ path: 'docs/screenshots/components/protected-route/03-active.png', fullPage: true })`. Keep the test setup deterministic, avoid credentials in screenshots, and update images when the observable states change.
