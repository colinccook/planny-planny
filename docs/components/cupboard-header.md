...
Reusable UI documentation for **!CupboardHeader**.
...
- [Authoritative source: `CupboardHeader.tsx`](../src/components/store-cupboard/CupboardHeader.tsx)
- [Search all `!CupboardHeader` references](https://github.com/colinccook/planny-planny/search?q=!CupboardHeader&type=code)
- [Find tests that render or drive it](https://github.com/colinccook/planny-planny/search?q=!CupboardHeader+repo%3Acolinccook%2Fplanny-planny&type=code)
- [Canonical !UiComponents index](../ui-components.md)

## Dependencies

- No direct imports; the component uses browser/React primitives.
...
Document screenshots for each state that users can encounter. Capture at 390×844 and store them under `docs/screenshots/components/cupboard-header/`.
...
Playwright tests can create these documentation snapshots with `await page.screenshot({ path: 'docs/screenshots/components/cupboard-header/03-active.png', fullPage: true })`. Keep the test setup deterministic, avoid credentials in screenshots, and update images when the observable states change.
