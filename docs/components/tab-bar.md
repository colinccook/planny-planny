...
Reusable UI documentation for **!TabBar**.
...
- [Authoritative source: `TabBar.tsx`](../src/components/layout/TabBar.tsx)
- [Search all `!TabBar` references](https://github.com/colinccook/planny-planny/search?q=!TabBar&type=code)
- [Find tests that render or drive it](https://github.com/colinccook/planny-planny/search?q=!TabBar+repo%3Acolinccook%2Fplanny-planny&type=code)
...
Document screenshots for each state that users can encounter. Capture at 390×844 and store them under `docs/screenshots/components/tab-bar/`.
...
Playwright tests can create these documentation snapshots with `await page.screenshot({ path: 'docs/screenshots/components/tab-bar/03-active.png', fullPage: true })`. Keep the test setup deterministic, avoid credentials in screenshots, and update images when the observable states change.
