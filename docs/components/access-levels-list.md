...
Reusable UI documentation for **!AccessLevelsList**.
...
- [Authoritative source: `AccessLevelsList.tsx`](../src/components/settings/AccessLevelsList.tsx)
- [Search all `!AccessLevelsList` references](https://github.com/colinccook/planny-planny/search?q=!AccessLevelsList&type=code)
- [Find tests that render or drive it](https://github.com/colinccook/planny-planny/search?q=!AccessLevelsList+repo%3Acolinccook%2Fplanny-planny&type=code)
...
Document screenshots for each state that users can encounter. Capture at 390×844 and store them under `docs/screenshots/components/access-levels-list/`.
...
Playwright tests can create these documentation snapshots with `await page.screenshot({ path: 'docs/screenshots/components/access-levels-list/03-active.png', fullPage: true })`. Keep the test setup deterministic, avoid credentials in screenshots, and update images when the observable states change.
