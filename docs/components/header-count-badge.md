...
Reusable UI documentation for **!HeaderCountBadge**.
...
- [Authoritative source: `HeaderCountBadge.tsx`](../src/components/ui/HeaderCountBadge.tsx)
- [Search all `!HeaderCountBadge` references](https://github.com/colinccook/planny-planny/search?q=!HeaderCountBadge&type=code)
- [Find tests that render or drive it](https://github.com/colinccook/planny-planny/search?q=!HeaderCountBadge+repo%3Acolinccook%2Fplanny-planny&type=code)
...
Document screenshots for each state that users can encounter. Capture at 390×844 and store them under `docs/screenshots/components/header-count-badge/`.
...
Playwright tests can create these documentation snapshots with `await page.screenshot({ path: 'docs/screenshots/components/header-count-badge/03-active.png', fullPage: true })`. Keep the test setup deterministic, avoid credentials in screenshots, and update images when the observable states change.
