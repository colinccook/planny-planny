...
Reusable UI documentation for **!PreferencesSettings**.
...
- [Authoritative source: `PreferencesSettings.tsx`](../src/components/settings/PreferencesSettings.tsx)
- [Search all `!PreferencesSettings` references](https://github.com/colinccook/planny-planny/search?q=!PreferencesSettings&type=code)
- [Find tests that render or drive it](https://github.com/colinccook/planny-planny/search?q=!PreferencesSettings+repo%3Acolinccook%2Fplanny-planny&type=code)
...
Document screenshots for each state that users can encounter. Capture at 390×844 and store them under `docs/screenshots/components/preferences-settings/`.
...
Playwright tests can create these documentation snapshots with `await page.screenshot({ path: 'docs/screenshots/components/preferences-settings/03-active.png', fullPage: true })`. Keep the test setup deterministic, avoid credentials in screenshots, and update images when the observable states change.
