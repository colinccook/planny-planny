...
Reusable UI documentation for **!CopyMealTray**.
...
- [Authoritative source: `CopyMealTray.tsx`](../src/components/calendar/CopyMealTray.tsx)
- [Search all `!CopyMealTray` references](https://github.com/colinccook/planny-planny/search?q=!CopyMealTray&type=code)
- [Find tests that render or drive it](https://github.com/colinccook/planny-planny/search?q=!CopyMealTray+repo%3Acolinccook%2Fplanny-planny&type=code)
...
Document screenshots for each state that users can encounter. Capture at 390×844 and store them under `docs/screenshots/components/copy-meal-tray/`.
...
Playwright tests can create these documentation snapshots with `await page.screenshot({ path: 'docs/screenshots/components/copy-meal-tray/03-active.png', fullPage: true })`. Keep the test setup deterministic, avoid credentials in screenshots, and update images when the observable states change.
