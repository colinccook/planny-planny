...
Reusable UI documentation for **!DeleteAccount**.
...
- [Authoritative source: `DeleteAccount.tsx`](../src/components/settings/DeleteAccount.tsx)
- [Search all `!DeleteAccount` references](https://github.com/colinccook/planny-planny/search?q=!DeleteAccount&type=code)
- [Find tests that render or drive it](https://github.com/colinccook/planny-planny/search?q=!DeleteAccount+repo%3Acolinccook%2Fplanny-planny&type=code)
...
Document screenshots for each state that users can encounter. Capture at 390×844 and store them under `docs/screenshots/components/delete-account/`.
...
Playwright tests can create these documentation snapshots with `await page.screenshot({ path: 'docs/screenshots/components/delete-account/03-active.png', fullPage: true })`. Keep the test setup deterministic, avoid credentials in screenshots, and update images when the observable states change.
