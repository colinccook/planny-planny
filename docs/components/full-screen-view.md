...
Reusable UI documentation for **!FullScreenView**.
...
- [Authoritative source: `FullScreenView.tsx`](../src/components/ui/FullScreenView.tsx)
- [Search all `!FullScreenView` references](https://github.com/colinccook/planny-planny/search?q=!FullScreenView&type=code)
- [Find tests that render or drive it](https://github.com/colinccook/planny-planny/search?q=!FullScreenView+repo%3Acolinccook%2Fplanny-planny&type=code)
...
Document screenshots for each state that users can encounter. Capture at 390×844 and store them under `docs/screenshots/components/full-screen-view/`.
...
Playwright tests can create these documentation snapshots with `await page.screenshot({ path: 'docs/screenshots/components/full-screen-view/03-active.png', fullPage: true })`. Keep the test setup deterministic, avoid credentials in screenshots, and update images when the observable states change.
