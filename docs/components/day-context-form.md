...
Reusable UI documentation for **!DayContextForm**.
...
- [Authoritative source: `DayContextForm.tsx`](../src/components/calendar/DayContextForm.tsx)
- [Search all `!DayContextForm` references](https://github.com/colinccook/planny-planny/search?q=!DayContextForm&type=code)
- [Find tests that render or drive it](https://github.com/colinccook/planny-planny/search?q=!DayContextForm+repo%3Acolinccook%2Fplanny-planny&type=code)
...
Document screenshots for each state that users can encounter. Capture at 390×844 and store them under `docs/screenshots/components/day-context-form/`.
...
Playwright tests can create these documentation snapshots with `await page.screenshot({ path: 'docs/screenshots/components/day-context-form/03-active.png', fullPage: true })`. Keep the test setup deterministic, avoid credentials in screenshots, and update images when the observable states change.
