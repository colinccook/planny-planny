...
Reusable UI documentation for **!DayHeaderStrip**.
...
- [Authoritative source: `DayHeaderStrip.tsx`](../src/components/calendar/DayHeaderStrip.tsx)
- [Search all `!DayHeaderStrip` references](https://github.com/colinccook/planny-planny/search?q=!DayHeaderStrip&type=code)
- [Find tests that render or drive it](https://github.com/colinccook/planny-planny/search?q=!DayHeaderStrip+repo%3Acolinccook%2Fplanny-planny&type=code)
...
Document screenshots for each state that users can encounter. Capture at 390×844 and store them under `docs/screenshots/components/day-header-strip/`.
...
Playwright tests can create these documentation snapshots with `await page.screenshot({ path: 'docs/screenshots/components/day-header-strip/03-active.png', fullPage: true })`. Keep the test setup deterministic, avoid credentials in screenshots, and update images when the observable states change.
