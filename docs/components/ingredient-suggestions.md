...
Reusable UI documentation for **!IngredientSuggestions**.
...
- [Authoritative source: `IngredientSuggestions.tsx`](../src/components/ingredients/IngredientSuggestions.tsx)
- [Search all `!IngredientSuggestions` references](https://github.com/colinccook/planny-planny/search?q=!IngredientSuggestions&type=code)
- [Find tests that render or drive it](https://github.com/colinccook/planny-planny/search?q=!IngredientSuggestions+repo%3Acolinccook%2Fplanny-planny&type=code)
...
Document screenshots for each state that users can encounter. Capture at 390×844 and store them under `docs/screenshots/components/ingredient-suggestions/`.
...
Playwright tests can create these documentation snapshots with `await page.screenshot({ path: 'docs/screenshots/components/ingredient-suggestions/03-active.png', fullPage: true })`. Keep the test setup deterministic, avoid credentials in screenshots, and update images when the observable states change.
