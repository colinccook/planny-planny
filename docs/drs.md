...
| #ChatGptPlugin | [dr-021](drs/dr-021-mcp-oauth-compatibility-bridge.md) | Keep ChatGPT's published `/mcp` resource on native Supabase OAuth, while a separate Claude MCP resource temporarily uses a hardened compatibility bridge that still issues native Supabase session tokens for JWT verification, RLS and role checks. |
